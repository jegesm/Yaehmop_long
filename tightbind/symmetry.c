  ...
  long show_equiv;
{
  ...
  case Rotation:
    fprintf(the_file,"%d-Fold Rotation about (%5.3lf,%5.3lf,%5.3lf)\n",
	    (long)(TWOPI/elem->angle), elem->axis.x,elem->axis.y,elem->axis.z);
    break;
...
void find_off_axis_sym_ops(detail_type *details,cell_type *cell,
			   ...
			   sym_op_type *the_ops,long *num_ops)
{
  ...
  ********/
  ...
  while(this_op){
    ...
    this_op->equiv_atoms = (long *)calloc(cell->num_atoms,sizeof(long));
	...
	printf("a -> (%d %d %d)\n",(long)mapping[0],(long)mapping[1],(long)mapping[2]);
	printf("b -> (%d %d %d)\n",(long)mapping[3],(long)mapping[4],(long)mapping[5]);
	printf("c -> (%d %d %d)\n",(long)mapping[6],(long)mapping[7],(long)mapping[8]);
      }
#endif
    }
  ...
  cell_type *cell;
{
  ...
  *********/  
  ...
  while(this_op){
    ...
    this_op->equiv_atoms = (long *)calloc(num_atoms,sizeof(long));
    if( !(this_op->equiv_atoms) ) fatal("Can't get memory for equiv_atom array.");
  ...
  /* now prlong out the symmetry operations that are present */
...
{  
  ...
  long i,j,k,ops_so_far;
  long num_atoms;
  long atom1,atom2;
  long begin_atom1,end_atom1,begin_atom2,end_atom2;
