...
*
*****************************************************************************/
void gen_k_point_mesh(point_type **points,long num_per_vect[3],
		      long *num_generated, char include_high_symm_p,long dim,
		      real offset)
{
  long i,j,k;
  long num_p;
...
void automagic_k_points(detail_type *details,cell_type *cell)
{
  ...
  real *degen_p;
  long num_raw_points,num_points;
  real tot_weight;
  long nonorthogonal_lattice;
	long num_duplicates;
  ...
  /********
    ...
    that's in hand, now we can reduce the k_polong set
...
#ifdef DEBUG_SYMM
  printf("Here's the reducible k-polong mesh (%d duplicates removed) for (%d %d %d) points per axis,\n",
  ...
  *******/
  ...
  if( nonorthogonal_lattice ){
    ...
    printf("\tminimal k-polong set for non-orthogonal lattices.\n");
    printf("\tI am going to make an attempt to reduce the zone somewhat,\n");
    printf("\tbut you should check the resulting k-polong set *carefully*.\n");
    printf("\tFor a more efficent sampling, you will have to use one of the\n");
    printf("\tdistributed k-polong programs.\n");
    ...
    fprintf(status_file,"\tminimal k-polong set for non-orthogonal lattices.\n");
    fprintf(status_file,
	    "\tI am going to make an attempt to reduce the zone somewhat,\n");
    fprintf(status_file,
	    "\tbut you should check the resulting k-polong set *carefully*.\n");
    fprintf(status_file,
	    "\tfor a more efficent sampling, you will have to use one of the\n");
    fprintf(status_file,"\tdistributed k-polong programs.\n");
    ...
    fprintf(output_file,";\tminimal k-polong set for non-orthogonal lattices.\n");
    fprintf(output_file,
	    ";\tI am going to make an attempt to reduce the zone somewhat,\n");
    fprintf(output_file,
	    ";\tbut you should check the resulting k-polong set *carefully*.\n");
    fprintf(output_file,
	    ";\tfor a more efficent sampling, you will have to use one of the\n");
    fprintf(output_file,";\tdistributed k-polong programs.\n");
