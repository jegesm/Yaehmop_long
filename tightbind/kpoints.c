 ...
 * Returns: none
 *
 * Action:  Loops over all the k-points for this run.  At each k-polong the following
 *     things are done:
 *    - Generate the K space overlap and hamiltonian matrices.
 *    - solve the eigenvalue eqn:
 *      H(k) * Y = S(k) * E * Y
 *    - do any required calculations for this k-polong (properties, etc.)
  ...
  long *orbital_lookup_table;
{
  ...
  long electrons_so_far;
  real *occupations;
  real *chg_mat;
  long num_KPOINTS;
  long overlap_file,hamil_file;
#ifdef USE_LAPACK
  char jobz, uplo;
  long info, itype;
  long num_orbs2;
  ...
  /********
    ...
    here's the loop over the k polong set.
...
#pragma pfor iterate (i=0;num_KPOINTS;1)
  ...
  for(i=0;i<num_KPOINTS;i++){
    /* get a pointer to the k polong we're working on */
    kpoint = &(details->K_POINTS[i]);
    
    /* prlong some status information */
    if( cell->dim > 0){
	...
	*******/
	...
	write(overlap_file,(const char *)&num_KPOINTS,sizeof(long));
	write(overlap_file,(const char *)&num_orbs,sizeof(long));	
	...
	write(hamil_file,(const char *)&num_KPOINTS,sizeof(long));
	write(hamil_file,(const char *)&num_orbs,sizeof(long));	
