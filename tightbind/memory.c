...
*
*****************************************************************************/
long *my_malloc(long long size)
{
  long *tptr;

  fprintf(stderr,"allocating %ld bytes\n",size);
  tptr = (long *)malloc(size);
  if( !tptr ){
...
*
*****************************************************************************/
long *my_calloc(long long num, long size)
{
  long *tptr;

  tptr = (long *)calloc(num,size);
  if( !tptr ){
...
*
*****************************************************************************/
long *my_realloc(long *ptr, long size)
{
  long *tptr;

  tptr = (long *)calloc(1,size);
  if( !tptr ){
  ...
  K_orb_ptr_type **orbital_ordering;
{
  ...
  long begin,end;
  long num_so_far;
  long long mem_for_avg_props;
  long long mem_per_overlapR,mem_per_hamR;
  long long mem_per_overlapK,mem_per_hamK;
  ...
  *******/
  if( (details->num_FMO_frags != 0 || details->num_FCO_frags != 0) &&
    ...
    for(i=0;i<num_frags;i++){
      ...
      FMO_frag->orbital_lookup_table = (long *)my_calloc(FMO_frag->num_atoms,
						     sizeof(long));
...
//CSABAI
fprintf(stderr,"num_orbs:%ld  tot_overlaps:%ld\n",num_orbs,(*tot_overlaps));
fprintf(stderr,"details->Execution_Mode: %d\n",details->Execution_Mode);
...
fprintf(stderr,"details->Execution_Mode: %d\n",details->Execution_Mode);
//CSABAI
    fprintf(stderr,"calling alloc for %lld bytes\n",mem_per_hamR);
