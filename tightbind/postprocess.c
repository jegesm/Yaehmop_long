  ...
  long *orbital_lookup_table;
{
  ...
  /**************
    ...
    do the FMO transformation and prlong out the results
  ...
  if( details->num_FMO_frags != 0 ){
    ...
    /* do we need to prlong out the overlap matrix? */
  ...
  long *orbital_lookup_table;
{
  ...
  if( details->Execution_Mode != MOLECULAR &&
    ...
    *******/
	...
	we start with an long of known value to allow us to
	check at read time that things are fine (i.e. it's a similar 
	machine type).  Otherwise fit_FCO will read garbage.

      **************/
      test_int = 4231;
      write(FCO_file,(const char *)&(test_int),sizeof(long));
      write(FCO_file,(const char *)&(details->num_KPOINTS),sizeof(long));
      tot_K_weight = 0.0;
      for(i=0;i<details->num_KPOINTS;i++){
	tot_K_weight += details->K_POINTS[i].weight;
      }
      write(FCO_file,(const char *)&(tot_K_weight),sizeof(real));
      write(FCO_file,(const char *)&(num_orbs),sizeof(long));
      write(FCO_file,(const char *)&(details->num_FCO_frags),sizeof(long));
      for(i=0;i<details->num_FCO_frags;i++){
	write(FCO_file,(const char *)&(details->FMO_frags[i].num_orbs),sizeof(long));
      }
    }
    ...
    /**************
      ...
      do the FCO transformation and prlong out the results
 ...
 *   putting it all in here makes the k polong loop a lot cleaner
  ...
  long *orbital_lookup_table;
{
  ...
  /* do we need to prlong out the wave functions? */
  if( details->wave_fn_PRT ){
...
INCREASE to the right)\n");
    ...
    /********
      ...
      there's no need to prlong out imaginary components of the
  ...
  /* prlong out the energies and the total energy */
  total_energy = 0;
...
*/
    for(j=0;j<num_orbs;j++){
      fprintf(output_file,"%d:--->  %16.12lg  [%4.3lf Electrons]\n",j+1,
              EIGENVAL(eigenset,j), occupations[j]);
      total_energy += occupations[j]*EIGENVAL(eigenset,j);
    }
    fprintf(output_file,"Total_Energy: %16.12lg\n",total_energy);
    properties->total_E = total_energy;
  }
