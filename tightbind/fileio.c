 ...
 *
 *****************************************************************************/
void write_atom_coords(atom_type *atoms,long num_atoms,
 ...
 *
 *****************************************************************************/
void write_atom_parms(detail_type *details,atom_type *atoms,long num_atoms,
                      char print_them)
{
  static long first_call=1,max_num_unique=0;
  atom_type *atom;
  long i,j;
  char found_this_one;
  ...
  for( i=0; i<num_atoms; i++ ){
    ...
    if( !found_this_one ){
      ...
      if( num_unique_atoms == max_num_unique ){
        max_num_unique += num_atoms;
        unique_atoms = (atom_type *)my_realloc((long *)unique_atoms,
  ...
  cell_type *cell;
{
  ...
  for(i=0;i<num_equiv;i++){
    ...
    equiv_list->equiv_atoms = (long *)calloc(num_read,sizeof(long));
    if( !equiv_list->equiv_atoms ) fatal("Can't get memory for equiv_atoms array");

    /* now add the entries to the array */
    bcopy(values_read,equiv_list->equiv_atoms,num_read*sizeof(long));
  ...
  FILE *the_file;
{
  ...
  FILE *infile;
  k_point_type *points;
  long num_k_points,max_k_points;
  real *real_ptr;
  long *int_ptr,*iarray;
  long reading_atom_nums;  
  long idle;
  real ecut,eerr;
  char got_geom;
  ...
  long which,foo_int,tot_num_so_far;
  long num_COOPS;
...
//CSABAI
 ...
 evaluated.\n");
	...
	**************/
	...
	fprintf(status_file,"An automatic k-polong mesh (%dx%dx%d) will be used.\n",
	...
	if( details->Execution_Mode == MOLECULAR ){
	  fprintf(status_file,"Warning: a K polong set was specified for a molecular \
calculation.\n");
	}
	...
	if(!points)fatal("Can't allocate memory for k polong set.");
	...
	for(i=0;i<details->num_proj_DOS;i++){
	  ...
	  p_DOS->contributions = (long *)calloc(max_p_DOS,sizeof(long));
	  ...
	  ****/
	  ...
	  while(!done){
		...
		bcopy((char *)int_ptr,(char *)p_DOS->contributions,
		      p_DOS->num_contributions*sizeof(long));
	      }
	    }
	  }		 
	}
	...
	/* get memory */
	details->MOs_to_print = (long *)calloc(details->num_MOs_to_print,
					      sizeof(long));
	...
	if(!band_info->special_points)
	  fatal("Can't get space for special polong storage.");
