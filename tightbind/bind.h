...
/* offset from high symmetry points for automatic k-polong generation */
...
typedef struct{
  ...
  long num_dim_ID,space_dim_ID,name_len_ID,param_len_ID;
} netCDF_info_type;
#endif
...
typedef struct{
  ...
  long ns,np,nd,nf;  /* principle quantum numbers */
...
************/
typedef struct{
  long dim;  /* dimension of the cell */
  long num_atoms,num_raw_atoms;
  atom_type *atoms;
...
************/
typedef struct{
  long num_special_points;
  long points_per_line;
...
struct printing_info_def {
  ...
  long contrib1,contrib2; /* used to indicate orbitals or atoms or FMOs */
...
********************/
typedef struct{
  long type; /* what kind of DOS it will be */
...
typedef struct{
  ...
  ********/
  ...
  long upper_level_PRT,lower_level_PRT; 
  ...
  ******/
  ...
  long points_per_axis[3];
  real k_offset;

  /* multiple occupations at each k polong */
  long num_occup_KPOINTS;
  real *occup_KPOINTS;
...
extern long num_orbs,tot_overlaps;
extern long *orbital_lookup_table;
extern atom_type *unique_atoms;
extern long num_unique_atoms;
