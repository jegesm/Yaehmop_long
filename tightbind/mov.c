...
void mov(sigma,pi,delta,phi,which1,which2,dist,q_num1,q_num2,
  ...
  long which1,which2,q_num1,q_num2,l1,l2;
  atom_type *atoms;
{
  long i,j,num_zeta1,num_zeta2;
  real coeff_1,coeff_2,sk1,sk2,r;
  
  real A_fn_values[30], B_fn_values[30];
  real ang_ind_overlap[4];
  
  long loopvar,max, m=0, nn;  /* definitions for abfns.f and lovlap.f */
  max = q_num1 + q_num2;
  ...
  *sigma=*pi=*delta=*phi=0.0;
  ...
  /* test prlong of A and B functions */
