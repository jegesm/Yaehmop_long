...
char test_string[80];  
  long zeta_converged,Hii_converged;
  ...
  ***********/
  for( walsh_step=0; walsh_step<details->walsh_details.num_steps; walsh_step++){
	  ...
	  *********/
	  if( details->avg_props ){
		...
		*******/
		...
		prlong out the stuff that will appear at the bottom of the file.
		...
		*************/
	      ...
	      /* prlong the moments if we generated them */
		...
		for(i=0;i<=details->num_occup_AVG;i++){
		  ...
		  /********
		    ...
		    now prlong out the average values of the COOP's that
		    the user asked for
	...
	**********/
    ...
    /******
      prlong out any important walsh results 
      ******/
    if( details->walsh_details.num_vars != 0 ){
      if( details->Execution_Mode != MOLECULAR ){
