  ...
  real *AO_occups;
{
  ...
  ******/
  ...
  for(i=0;i<num_orbs;i++) fprintf(output_file,"%d ",
				  (long)details->K_POINTS[i].num_filled_bands);
  fprintf(output_file,"\n");
  ...
  prop_type properties;
{
  ...
  real num_electrons;
  long num_elements,num_occup_bands,num_occup_orbs;
  COOP_type *COOP_ptr;
  ...
  real *Fermi_E;
{
  long i;
  long tot_orbs;
  long last_occup,begin_degen,end_degen,num_degen_levels;
