 ...
 *   fails.
 */
void fatal_bug( char *errorstring, char *file, long line )
{
 ...
 *   fails.
 */
void nonfatal_bug( char *errorstring, char *file, long line )
{
...
*  
*****************************************************************************/
void center_text_string(char *src, char *dest, long dest_len)
{
  long src_len,shift,i;

  src_len = strlen(src);
  if( dest_len < src_len ) FATAL_BUG("center_text_string got a src longer than dest");
  shift = (long)floor((float)(dest_len-src_len)/2);
...
*  
*****************************************************************************/
void left_just_text_string(char *src, char *dest, long dest_len)
{
  long src_len,shift,i;
...
*  
*****************************************************************************/
void map_orb_num_to_name(char *name,long orb_num,long *orbital_lookup_table,
			 long num_orbs,atom_type *atoms,long num_atoms)
{
  long i,j,orb_offset;
  atom_type *atom;
...
void debugmat( mat, num_row, num_col, tol )
  real *mat;
  long num_row,num_col;
  real tol;
{
  long i,j;
  long beg_col,end_col;
  long which_part,num_parts;
  real val;
...
*
*****************************************************************************/
void printmat( real *mat, long num_row, long num_col, FILE *outfile, real tol,
	      char transpose,long width)
{
  long i,j;
  long beg_col,end_col;
  long beg_row,end_row;
  long which_part,num_parts;
  long cols_per_line;
  real val;
  
  /* figure out how many columns we get.... */
  cols_per_line = (long)floor((real)width/10);
...
*
*****************************************************************************/
void print_labelled_mat( real *mat, long num_row, long num_col, FILE *outfile, real tol,
			atom_type *atoms,long num_atoms, long *orbital_lookup_table,
			long num_orbs,char transpose,char label_which,long width)
{
  ...
  /* figure out how many columns we get.... */
  cols_per_line = (long)floor((real)width/18);
...
*
*****************************************************************************/
void print_sym_mat( real *mat, long num_row, long num_col, FILE *outfile, char *legend,
		   char *titles,long width)
{
  long i,j;
  long beg_col,end_col;
  long which_part,num_parts;
  ...
  for(which_part=0;which_part<num_parts;which_part++){
	...
	/* go ahead and prlong out the values for this row */
 ...
 *
 ****************************************************************************/
long skipcomments(FILE *file,char *string,char toggle)
{
  long i;
...
void find_atoms_orbs(num_orbs,num_atoms,atom,orbital_lookup_table,begin,end)
  long num_orbs,num_atoms;
  long atom;
  long *orbital_lookup_table;
  long *begin,*end;
{
  char done;
  long next_real_atom;
...
long overlap_tab_from_vect(vect,cell)
  point_type *vect;
  cell_type *cell;
{
  long x,y,z;
...
*
*  The polong of this is to prevent trap as many as possible things
...
/*  cell->num_atoms = num_atoms; */
  ...
  *orbital_lookup_table = (long *)calloc(cell->num_atoms,sizeof(long));
...
void print_MOs(details,num_orbs,eigenset,kpoint,unique_atoms,num_unique_atoms,
  ...
  long num_unique_atoms;
  long num_atoms;
  long *orbital_lookup_table;
{
  static char first_call=1;
  static long x_mirror_present,y_mirror_present,z_mirror_present;
  char filename[240];

  long i,j,k;
  long curr_atom,begin_atom,end_atom;
  long num_walsh_steps;
  sym_op_type *symm_op;
  long ops_passed;
  real this_character;
  ...
  long **values,*num_values;
{
  ...
  *values = (long *)calloc(max_values,sizeof(long));
  ...
  while(num_string[0]){
	...
	max_values += 10;
	*values = (long *)my_realloc(*values,max_values*sizeof(long));
...
void dump_hermetian_mat(file,mat,num_orbs)
  ...
  long num_orbs;
{
  write(file,(const char *)mat,(long long)num_orbs*(long long)num_orbs*sizeof(real));
}
 ...
 ****************************************************************************/
// CSABAI
// this is the original yaEHMOP version
void dump_sparse_mat_orig(file,mat,num_orbs,cut_off)
  FILE *file;
  real *mat;
  long num_orbs;
  real cut_off;
...
// then only the non-zero elements are written in 3 columns the two indces and the value: i j x_ij
  ...
  real cut_off;
{
  ...
  fprintf(stderr,"%ld hermetian matrix rows/coulmns with %lf cutoff\n",
	  num_orbs,cut_off);

  fprintf(file,"%ld %ld 0.0\n",num_orbs,num_orbs);
  /* write the nonzero elements */
  for(i=0;i<num_orbs;i++){
    itab = i*num_orbs;
    for(j=0;j<num_orbs;j++){
