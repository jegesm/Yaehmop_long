  ...
  *****/
  ...
  for(i=0;i<details->num_proj_DOS;i++){
    ...
    *******/
    ...
    p_DOS->contributions = (long *)calloc(p_DOS->num_contributions,sizeof(long));
...
contributions.");
  ...
  **********/
  if( cell->dim > 0 ){
	...
	if( !points )fatal("Can't reallocate k polong array to get more space.");
      }
    }
