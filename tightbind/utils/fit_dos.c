...
#endif
{
  ...
  real broadening;
  long num_E_steps;
  long done,num,num_curves,max_curves;
  long points_per_DOS;
  long num_DOS,max_DOS;
  long which_int;
  real fermi;
  long eof_hit;
  real norm_fact;
  long num_walsh_steps;
  long which_walsh_step;
  long temp;
...
//  argc = ccommand(&argv);
  ...
  *******/
  ...
  max_curves = 10;
  num_curves = 0;
  num_states = (long *)calloc(max_curves,sizeof(int));
  ...
  *******/
  ...
  /******
    ...
    now prlong out the integration data
