...
long sort_FCO_helper(const void *p1,const void *p2)
{
...
#endif
{
  ...
  long i,j,k,itab,jtab,ktab,points_per_side;
  long which_frag,orbs_this_frag;

long long calls_to_exp = 0;
 #ifdef USING_THE_MAC
  long argc;
  char argv[4][80];
...
//  argc = ccommand(&argv);
  ...
  /* first read the test long to make sure things are hunky dorey */
  read(FCO_file,(const char *)&test_int,sizeof(int));
  if( test_long != 4231 ){
    fprintf(stderr,"fit_FCO just failed while reading in the test long value.\n");
    fprintf(stderr,"  the polong of this is to make sure the binary file you give\n");
  ...
  FCO_num_orbs = (long *)calloc(num_FCO_frags,sizeof(int));
  ...
  if( !charge_mat || !frag_Es ) fatal("Can't get K polong memory");
  ...
  ********/
  ...
  for(k=0;k<num_Kpoints;k++){
	...
	/* make sure the polong falls in our energy window */
