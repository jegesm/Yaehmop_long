...
long find_atom(atoms,num_atoms,which)
  atom_type *atoms;
  long num_atoms,which;
{
...
*
*****************************************************************************/
void eval_Zmat_locs(atom_type *atoms,long num_atoms, long cell_dim,char printing)
{
  long i,j;
  long which_atom,which_atom2;
  atom_type *atom;
  point_type *loc;
  Z_mat_type *Zmat_loc;
  ...
  *******/
  ...
  if( printing ){
    /* now prlong out the positions of the atoms */
